...
exports.ban = async function (message) {
    ...
    fs.appendFileSync('./lib/banned.json', `${message.from} ${numBan}@c.us\n`)
    console.log(numBan)
}

exports.unban = async function (message) {
    numUnBan = message.body.substring(message.body.indexOf('@') + 1)
    fs.readFile('./lib/banned.json', { encoding: 'utf-8'}, function (err, data) {
        if (err) throw err
        ...
        fs.writeFile('./lib/banned.json', dataUpdate, function(err) {
            if (err) throw err
        })
    })
}
